...
_INTERNAL_AGENTS = {
    "trace_agent", "ida_jadx_agent", "frida_agent", "network_agent",
    "apktool_agent", "js_reverse_agent", "ida_agent",
    "unidbg_agent",
}
...
- `@trace_agent` — ARM64 执行流分析
- `@ida_jadx_agent` — Java 静态代码分析（JADX 反编译）
- `@frida_agent` — 动态分析（Frida Hook/类枚举）
- `@network_agent` — 网络流量分析（HTTP 请求/签名定位）
- `@apktool_agent` — APK 解包/重打包/Smali 分析
- `@js_reverse_agent` — WebView JS 反混淆/格式化
- `@ida_agent` — Native 深度分析（IDA Pro）
- `@unidbg_agent` — Native SO 模拟执行与算法复现
